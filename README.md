...
* **Advanced Decoding Engines:** Supports both **Greedy Search** and **Beam Search** with customizable beam width ($k$) and length normalization ($\alpha$).
...
python scripts/evaluate.py \
    ...
    --output_path figures/attention_map.png
```
...
| **Dropout ($P_{\text{drop}}$)**          |       0.1      |     0.1    | Residual and attention dropout probability            |
...
| Transformer-Base (Beam + Length Norm) | 5 ($\alpha=0.6$) |   **38.4**   |       **1.58**       |  **4.85**  |
